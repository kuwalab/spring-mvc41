...
public class C026ControllerTest {
	@Autowired
	...
	public void uploadFormのGET() throws Exception {
		mockMvc.perform(get("/c026/uploadForm")).andExpect(status().isOk())
				.andExpect(view().name("c026/uploadForm"));
	}

	@Test
	public void uploadRecvのPOST() throws Exception {
		byte[] fileImage = null;
		Path path = Paths.get("src/test/resources/c026/kappa.png");
		...
		mockMvc.perform(
				fileUpload("/c026/uploadRecv").file(file).param("test",
